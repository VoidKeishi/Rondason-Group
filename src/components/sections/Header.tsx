import { LogoMark } from '@/components/ui/LogoMark';
import { NAV_LINKS } from '@/components/nav-links';
import { MobileMenu } from './MobileMenu';

export function Header() {
  return (
    <header className="sticky top-0 z-50 border-b border-line-light bg-white">
      <div className="relative flex items-center justify-between px-6 py-4 md:px-10 lg:px-16 lg:py-5">
        <a href="#home" className="flex items-center gap-3 text-navy">
          <LogoMark size={30} />
          <span className="font-display text-[17px] tracking-[1px]">
            RONDASON GROUP
          </span>
        </a>
        <nav className="hidden items-center gap-8 text-[13px] tracking-[0.5px] lg:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-ink transition-colors duration-200 hover:text-gold-deep"
            >
              {link.label}
            </a>
          ))}
          <a
            href="#contact"
            className="rounded-[2px] bg-navy px-[22px] py-[10px] font-semibold text-on-navy"
          >
            Get in Touch
          </a>
        </nav>
        <MobileMenu />
      </div>
    </header>
  );
}
